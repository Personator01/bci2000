...
# ///////////////////////////////////////////////////////////////////////////
# $Id: EventRelated.py 8030 2024-04-12 15:47:45Z jhill $
...
if __name__ == '__main__':
	...
	parser.add_argument( "-c", "--highlightChannel", metavar='CHANNEL',           default='auto',          help="which channel to select initially in plots: the default, 'auto', selects the one with the largest peak.  Append '+' or '-' to the name (or to 'auto') to restrict the search to positive or negative peaks. Append (e.g.) @300 to a specific channel name to select that channel at a particular millisecond latency" )
...
class EpochSet( Bunch ):
	...
	def __init__( self,
		...
		channelMap=None, defaultReference=None, rereference=None, highlightChannel='auto', superTitle='',
		keep=False,
		...
		if len( filenames ) == 1:
			...
			objs = [ EpochSet( filename,
				trigger=trigger, responseState=responseState,
				...
				channelMap=channelMap, defaultReference=defaultReference, rereference=rereference, highlightChannel=highlightChannel, superTitle=superTitle,
				keep=keep, plot=plotEach, plotEach=False, yScale=yScale,
		...
		if self.filterOrder and self.filterBand:
			filter = scipy.signal.butter( self.filterOrder, self.filterBand, btype='bandpass', fs=self.samplesPerSecond )
			bandpassSignal = scipy.signal.filtfilt( *filter, rawSignal, method='gust', axis=1 )
		else:
		...
		self.epochs, t, self.epochIndices = epochs(
			bandpassSignal,
			matchedEpochIndices,
			offset = -abs( lookBackMsec ) / 1000.0,
	...
	def Summary( self, select=None, select2=None, snr=False, baseline='auto', excludeBadEpochs=True, plot=False, highlightChannel='auto', superTitle='', minEpochsPerClass=0, yScale=None ):
		"""
		...
		result = Bunch(
			img = m,
			x = self.epochTimeMsec,
			xlabel = 'msec',
			colorbar = True,
			balance = 0.0,
			interactive = True,
			colorbartitle = 'SCD' if isSCD else 'z-score' if snr else r'$\Delta{} \mu{}$V' if n2 else r'$\mu{}$V', 
			title = self.Title() + '\n' + description,
		)
		...
		if plot != False:
			...
			if highlightChannel:
				...
				result.handle.select( peak[ 'peakChannelIndex' ], peak[ 'peakParamValue' ] )
	...
	def FindPeak( self, img, channels=None, paramRange=None, polarity=None ):
		...
		paramValues = self.epochTimeMsec
	...
	def Combine( cls, objs ):
		...
		for obj in objs[ 1: ]:
			...
			for field in 'hardwareEvents softwareEvents stimulusLatenciesMsec responseEvents epochs epochIndices trialType epochIsBad'.split():
				if len( obj[ field ] ): out[ field ] = numpy.concatenate( [ out[ field ], obj[ field ] ], axis=0 )
			for field in 'stimuli! channels! samplesPerSecond! usingHardwareEvents! epochTimeMsec! filterBand filterOrder badChannelThreshold badEpochThreshold triggerState'.split():
				strict = field.endswith( '!' )
				field = field.strip( '!' )
				a, b = out[ field ], obj[ field ]
