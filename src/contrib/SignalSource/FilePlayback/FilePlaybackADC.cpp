////////////////////////////////////////////////////////////////////////////////
// $Id: FilePlaybackADC.cpp 7923 2024-03-12 18:11:55Z jhill $
...
namespace {
    ...
    {
        for (auto name : {"FilePart", "Notes", })
            if (s.Name() == name)
                return true;
        return false;
    }

    bool IsLoggerParam(const Param& p)
    {
        const std::string& name = p.Name();
        for (const auto* pName : { "LogKeyboard", "LogMouse" })
            if (name == pName)
                return true;
        ...
        if (hl.size() >= 2)
        {
            const std::string blame = hl.back();
            if (blame.size() > 6 && blame.substr(blame.size() - 6) == "Logger")
                if (name.size() > 3 && name.substr(0, 3) == "Log")
                    return true; // skip anything whose name begins with "Log" if it was declared by a class whose name ends in "Logger".
...
//{
  ...
  if( mTemplateFileName.size() ) // --PlaybackFileName was given as a command-line option. Use this opportunity to declare all the parameters and states that are in the file, giving the file's parameter values as defaults (with a few exceptions, below).
  {
    ...
    {
      if (!IsPaddingState(state) && !IsIgnoreState(state))
      {
        std::stringstream ss;
        ss << state;
        std::string s = ss.str();
