////////////////////////////////////////////////////////////////////////////////
// $Id: sockstream.cpp 8091 2024-05-01 13:48:52Z mellinger $
...
{
    return build_address(address, m_address);
}

bool socket::set_address(const std::string &inIP, unsigned short inPort)
{
    return build_address(inIP, inPort, m_address);
}
...
std::string socket::ip() const
{
    if (m_fd < 0)
        return "<N/A>";
    return ip_from_address(m_address);
}
...
int socket::port() const
{
    if (m_fd < 0)
        return -1;
    return port_from_address(m_address);
}

std::string socket::address() const
{
    return address_to_string(m_address);
}
...
int sending_udpsocket::on_set_options()
{
    if (0 > udpsocket::on_set_options())
        return -1;
    int val = ::strcmp(::inet_ntoa(m_address.sin_addr), "255.255.255.255") ? 0 : 1; // Broadcast address
