...
void HyperscanningApplicationBase::Setup() {
	...
	send( mSocket.Fd(), ( const char* ) &size, sizeof( size_t ), NULL );
	send( mSocket.Fd(), version.c_str(), version.size(), NULL );
	...
	send( mSocket.Fd(), ( const char* ) &size, sizeof( size_t ), NULL );
	if ( size )
		send( mSocket.Fd(), id.c_str(), id.size(), NULL );
	...
	size = sid.size();
	send( mSocket.Fd(), ( const char* ) &size, sizeof( size_t ), NULL );
	send( mSocket.Fd(), sid.c_str(), sid.size(), NULL );
