...
class PACWindow(AbstractWindow):
    def newTrial(self):
    ...
    def giveUserInfo(self, text):
        ...
        self.binsGrid.clear()
        self.polarGrid.addLabel(text, **labArgs)
        self.binsGrid.addLabel(text, **labArgs)
    ...
    def updatePlots(self):
            try:
                self.bci2000State = self.bciThread.bci.GetSystemState()
                #if any(self.bci2000State == st for st in ["Idle", "Resting", "Suspended"]):

                if self.bci2000State != "Running":
                    self.initialized = False
                    if self.bci2000State == "Resting":
                        self.saveImages = False
                        self.giveUserInfo("Configuration set. Waiting for the run to be started...")
                    return
                else:
                ...
                #self.giveUserInfo("Lost connection with BCI2000")
                self.quitAll()
            else:
    ...
    def initialize(self):
        self.setWindowTitle("Phase-Amplitude Coupling")
        self.maxTrials = 5
        ...
        waitLabTxt = "Waiting for the run to be started..."
        # labArgs = {'color': "#FFFFFF",
        #            'size': "18pt",
        #            'bold': True}
        self.polarGrid = pg.GraphicsLayoutWidget(title=polarTitle)
        self.binsGrid = pg.GraphicsLayoutWidget(title="Bins Plot")
        #self.giveUserInfo(waitLabTxt)
        #self.polarGrid.addLabel(waitLabTxt, **labArgs)
        #self.binsGrid.addLabel(waitLabTxt, **labArgs)
        ...
        area.addDock(self.polarD, 'bottom')
        area.addDock(self.binsD, 'left', self.polarD)
    ...
    def __init__(self, size, bci, acq, title="Zmod Figure"):
        super().__init__(size, bci, acq)
