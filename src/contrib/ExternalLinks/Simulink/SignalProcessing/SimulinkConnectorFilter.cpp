////////////////////////////////////////////////////////////////////////////////
// $Id: SimulinkConnectorFilter.cpp 7813 2024-01-31 17:57:43Z mellinger $
//
...
void SimulinkConnectorFilter::Private::SenderThreadFunc()
{
    while (!mSenderThread.Terminating())
    {
        ...
        bciwarn << "Connected to " << socket.Address();
        ...
        if (socket.Connected() == Socket::local) // only use shared memory when connected locally
        {
            ...
            bciwarn << "Locally connected to " << socket.Address() << ", using shared memory";
        }
        else // otherwise, full data will be transmitted over network (slower)
        {
            bciwarn << "Remotely connected to " << socket.Address() << ", transmitting data through network";
        }
