/////////////////////////////////////////////////////////////////////////////
// $Id: SecondaryBCI2000Controller.cpp 7993 2024-04-04 20:44:05Z mellinger $
...
void SecondaryBCI2000Controller::Preflight() const
{
  ...
  for (auto p : { "SubjectName", "SubjectSession", "SubjectRun", "DataDirectory" })
  {
      std::string value = Parameter(p);
      mConnection.SetParameter(p, value);
  }
  std::string file = Parameter("DataFile");
  file = FileUtils::ExtractDirectory(file) + FileUtils::ExtractBase(file) + "_1" + FileUtils::ExtractExtension(file);
  
  mConnection.SetParameter("DataFile", file);
  // Make sure the secondary system does not do file splitting by itself. Instead, we will send
  // updates to the FilePart event.
