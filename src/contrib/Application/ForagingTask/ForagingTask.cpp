...
ForagingTask::Publish()
{
    ...
    "Application:Foraging%20Cursor int CursorSize= 5 % % % // Color of the cursor",
    ...
    "Application:Foraging%20Stimuli int MinimumAudioClicks= 5 0 0 % // Maximum number of trials that a color can miss in a row",
    "Application:Foraging%20Stimuli int MaximumAudioClicks= 75 1 1 % // Maximum number of trials that a color can miss in a row",
    ...
    "Application:Foraging%20Timing int ForagingTrialTime= 15s % % % // Length of Time that Subject has to forage",
		...
		"Application:Foraging%20Timing int ScoreFeedbackTime= 3000ms % % % // Length of time to show user score",
...
ForagingTask::Initialize( const SignalProperties& Input, const SignalProperties& Output )
{
  ...
  heightfactor = Parameter( "WindowWidth" ) / Parameter( "WindowHeight" );
	for ( int i = 0; i < m; i++ ) {
		for ( int p = 0; p < n; p++ ) {
			stimuli[ i + p * m ] = new EllipticShape( mrDisplay, 2 );
  ...
  //text for scoring in between trials
	...
	scoreText->SetDisplayRect(mIntroTxtRect);
	scoreText->SetTextColor(RGBColor::White);
	scoreText->SetAspectRatioMode(1);
	scoreText->Conceal();
