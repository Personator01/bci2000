...
void wcst::OnInitialize( const SignalProperties& inp ) {
	...
	text->SetDisplayRect( { 0.f, 0.f, 1.f, 1.f } );
	...
	topImage->SetDisplayRect( { 0.5f - x, 0.5f - ( 2 * y ) - spacey, 0.5f + x, 0.5f - spacey } );
	Associations()[ 1 ].Add( topImage );

	rightImage = new ImageStimulus( mrDisplay );
	rightImage->SetDisplayRect( { 0.5f + spacex, 0.5f - y, 0.5f + ( 2 * x ) + spacex, 0.5f + y } );
	Associations()[ 1 ].Add( rightImage );

	bottomImage = new ImageStimulus( mrDisplay );
	bottomImage->SetDisplayRect( { 0.5f - x, 0.5f + spacey, 0.5f + x, 0.5f + ( 2 * y ) + spacey } );
	Associations()[ 1 ].Add( bottomImage );

	leftImage = new ImageStimulus( mrDisplay );
	leftImage->SetDisplayRect( { 0.5f - ( 2 * x ) - spacex, 0.5f - y, 0.5f - spacex, 0.5f + y } );
	...
	fixationCross->SetDisplayRect( { 0.5f - x, 0.5f - y, 0.5f + x, 0.5f + y } );
