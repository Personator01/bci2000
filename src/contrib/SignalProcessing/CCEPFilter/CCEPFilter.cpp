...
CCEPFilter::Publish()
{
 ...
 BEGIN_PARAMETER_DEFINITIONS
   ...
   "Filtering:CCEP%20General%20Settings float ChannelThreshold= auto % % % "
      "// Magnitude threshold which defines a CCEP",
   "Filtering:CCEP%20General%20Settings string OnsetExpression= 0 % % % "
      "// Expression that defines stimulus onset",
   "Filtering:CCEP%20General%20Settings int Stimulator= 0 0 0 1 "
      "// Stimulator device: 0 auto, 1 NeuroOmega (enumeration) ",

   //Duration parameters
   "Filtering:CCEP%20Duration%20Settings int BaselineEpochLength= 0ms 0ms 0 % "
      "// Length of data epoch before stimulus onset",
   "Filtering:CCEP%20Duration%20Settings int TriggerLatencyLength= auto % 0 % "
      "// Latency of stimulation onset from stimulus onset",
   ...
   "Filtering:CCEP%20Counting%20Settings int HoldPlots= 20 20 1 % "
      "// Number of plots to display ",

   //Visualization settings
   "Filtering:CCEP%20Visualization int VisImageWidth= 200 % 0 % // native image width",
   "Filtering:CCEP%20Visualization int VisImageHeight= 200 % 0 % // native image height",
   "Filtering:CCEP%20Visualization int VisMaxWindows= 0 % 0 % // maximum number of windows, 0 for any",
...
{
  mStimulator = new StimulatorConfiguration(Parameter("Stimulator"));
  if (!mStimulator->validStimulator())
    return;
  Parameter("CCEPEpochLength") = mStimulator->getEpochLength();
  Parameter("ChannelThreshold") = "200muV";
  Parameter("TriggerLatencyLength") = "50ms";
...
CCEPFilter::Preflight( const SignalProperties& Input, SignalProperties& Output ) const
{
  ...
  double outputSamples = Parameter("CCEPEpochLength").InSampleBlocks() + Parameter("TriggerLatencyLength").InSampleBlocks();
  ...
  //visualization
  Parameter("VisImageWidth");
  Parameter("VisImageHeight");
  Parameter("ChannelThreshold");
