////////////////////////////////////////////////////////////////////////////////
// $Id: ProcessUtils.cpp 7706 2023-11-09 21:20:27Z mellinger $
...
#if !_WIN32
// Normalize name for POSIX semaphore
...
bool GlobalID::TryCreate(const std::string &inName)
{
    ...
    if (mpSemaphore != SEM_FAILED)
        return true;
    
    std::string name = NormalizeName(inName);
    mpSemaphore = ::sem_open(name.c_str(), O_CREAT | O_EXCL, 0666, 0);
    if (mpSemaphore != SEM_FAILED)
        mName = name;
...
void GlobalID::Destroy()
{
    ...
    if (mpSemaphore != SEM_FAILED)
    {
        ::sem_close(mpSemaphore);
        mpSemaphore = SEM_FAILED;
    }
    if (!mName.empty())
    {
...
void GlobalID::Cleanup(const std::string &inName)
{
    ...
    ::sem_unlink(NormalizeName(inName).c_str());
