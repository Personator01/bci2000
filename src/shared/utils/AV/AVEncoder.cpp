//////////////////////////////////////////////////////////////////////
// $Id: AVEncoder.cpp 7961 2024-03-26 16:02:46Z mellinger $
...
struct AVEncoder::Private
{
    ...
    void CloseFile()
    {
        ...
        if (mpFormatContext && !mStreams.empty())
        {
    ...
    void InitializeStream(int idx, int width, int height, Rate fps, int preset)
    {
        ...
        pCtx->bit_rate = 400000; // bogus value to satisfy h264 codec
...
AVEncoder& AVEncoder::OpenFile(const std::string& file)
{
    ...
    if (p->mWorkerThread.Run(call))
        p->mWorkerThread.Wait();
#else
    p->OpenFile(file);
#endif
    return *this;
...
AVEncoder& AVEncoder::InitializeVideoStream(int streamIndex, int width, int height, const Rate& fps, int preset)
{
#if USE_WORKER_THREAD
    MemberCall<void(Private*, int, int, int, Rate, int)> 
        call(&Private::InitializeStream, p, streamIndex, width, height, fps, preset);
    if (p->mWorkerThread.Run(call))
        p->mWorkerThread.Wait();
#else
    p->InitializeStream(streamIndex, width, height, fps, preset);
#endif
    return *this;
}
