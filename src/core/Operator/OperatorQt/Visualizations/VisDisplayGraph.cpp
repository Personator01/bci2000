////////////////////////////////////////////////////////////////////////////////
// $Id: VisDisplayGraph.cpp 7970 2024-03-27 14:01:51Z mellinger $
...
VisDisplayGraph::~VisDisplayGraph()
{
    ...
    Visconfigs()[mVisID].Get(CfgID::ValueUnitsVisible, valueUnitsVisibleByName);
    ContainerToMappedProperty(valueUnitsVisible, valueUnitsVisibleByName);
    Visconfigs()[mVisID].Put(CfgID::ValueUnitsVisible, valueUnitsVisibleByName, UserDefined);

    MappedProperty<double> userScalingByName;
    Visconfigs()[mVisID].Get(CfgID::ChannelUserScaling, userScalingByName);
    ContainerToMappedProperty(mChannelUserScaling, userScalingByName);
    Visconfigs()[mVisID].Put(CfgID::ChannelUserScaling, userScalingByName, UserDefined);
...
void VisDisplayGraph::OnSetConfig(ConfigSettings &inConfig)
{
    ...
    if (inConfig.Get(CfgID::MinValue, minValue))
        mpSignalWidget->Display().SetMinValue(minValue);
    if (inConfig.Get(CfgID::MaxValue, maxValue))
        mpSignalWidget->Display().SetMaxValue(maxValue);

    ValueList<double> minValues, maxValues;
    if (inConfig.Get(CfgID::MinValues, minValues))
        mpSignalWidget->Display().SetMinValues(minValues);
    if (inConfig.Get(CfgID::MaxValues, maxValues))
        mpSignalWidget->Display().SetMaxValues(maxValues);
    ...
    int channelGroupSize = mpSignalWidget->Display().ChannelGroupSize();
    if (inConfig.Get(CfgID::ChannelGroupSize, channelGroupSize))
    {
        if (channelGroupSize < 1)
            channelGroupSize = std::numeric_limits<int>::max();
        mpSignalWidget->Display().SetChannelGroupSize(channelGroupSize);
    }
    int graphType = mpSignalWidget->Display().DisplayMode();
    if (inConfig.Get(CfgID::GraphType, graphType))
        ...
        case CfgID::Polyline:
            ...
            break;
        case CfgID::Field2d:
            mpSignalWidget->Display().SetDisplayMode(SignalDisplay::field2d);
            break;
        }
    bool showNumericValues = mpSignalWidget->Display().NumericValuesVisible();
    if (inConfig.Get(CfgID::ShowNumericValues, showNumericValues))
        mpSignalWidget->Display().SetNumericValuesVisible(showNumericValues);

    bool showBaselines = mpSignalWidget->Display().BaselinesVisible();
    if (inConfig.Get(CfgID::ShowBaselines, showBaselines))
        mpSignalWidget->Display().SetBaselinesVisible(showBaselines);

    bool invertedDisplay = mpSignalWidget->Display().Inverted();
    if (inConfig.Get(CfgID::InvertedDisplay, invertedDisplay))
        mpSignalWidget->Display().SetInverted(invertedDisplay);

    int fontSize = mpSignalWidget->Display().FontSize();
    if (inConfig.Get(CfgID::FontSize, fontSize))
        mpSignalWidget->Display().SetFontSize(fontSize);
    ...
    std::string channelStyles = mpSignalWidget->Display().ChannelStyles();
    if (inConfig.Get(CfgID::ChannelStyles, channelStyles))
        mpSignalWidget->Display().SetChannelStyles(channelStyles);

    std::string valueColors;
    if (inConfig.Get(CfgID::ValueColors, valueColors))
        mpSignalWidget->Display().SetValueColors(valueColors);

    std::string unit;
    std::istringstream iss;
    if (inConfig.Get(CfgID::SampleUnit, unit))
    {
        double oldSampleUnit = NominalUnitsPerSample() * FilterUnitToValue(mpSignalWidget->Display().SampleUnit());
        PhysicalUnit u;
        u.SetGainWithSymbol(unit);
        float unitsPerSample = u.Gain();
        std::string sampleUnit = u.Symbol();
        SetNominalUnitsPerSample(unitsPerSample);
        mpSignalWidget->Display().SetSampleUnit(u.Symbol());
    ...
    if (inConfig.Get(CfgID::SampleOffset, sampleOffset))
        mpSignalWidget->Display().SetSampleOffset(-sampleOffset);
    ...
    if (inConfig.Get(CfgID::ChannelUnit, unit))
        mpSignalWidget->Display().SetChannelUnit(unit);

    if (inConfig.Get(CfgID::ValueUnit, unit))
        mpSignalWidget->Display().SetValueUnit(unit);

    ValueList<std::string> units = mpSignalWidget->Display().ValueUnits();
    if (inConfig.Get(CfgID::ValueUnits, units))
        mpSignalWidget->Display().SetValueUnits(units);

    bool showValueUnit = mpSignalWidget->Display().ValueUnitVisible();
    if (inConfig.Get(CfgID::ShowValueUnit, showValueUnit))
        mpSignalWidget->Display().SetValueUnitVisible(showValueUnit);

    bool xAxisLabels = mpSignalWidget->Display().XTicksVisible();
    if (inConfig.Get(CfgID::ShowXTicks, xAxisLabels))
        mpSignalWidget->Display().SetXTicksVisible(xAxisLabels);

    bool yAxisLabels = mpSignalWidget->Display().YTicksVisible();
    if (inConfig.Get(CfgID::ShowYTicks, yAxisLabels))
      mpSignalWidget->Display().SetYTicksVisible(yAxisLabels);

    LabelList labels = mpSignalWidget->Display().XAxisMarkers();
    if (inConfig.Get(CfgID::XAxisMarkers, labels))
        mpSignalWidget->Display().SetXAxisMarkers(labels);

    labels = mpSignalWidget->Display().YAxisMarkers();
    if (inConfig.Get(CfgID::YAxisMarkers, labels))
        mpSignalWidget->Display().SetYAxisMarkers(labels);

    inConfig.Get(CfgID::ChannelsVisible, mChannelsVisible);

    labels = mpSignalWidget->Display().ChannelLabels();
    if (inConfig.Get(CfgID::ChannelLabels, labels))
    {
        mpSignalWidget->Display().SetChannelLabels(labels);
        mpSignalWidget->Display().SetChannelLabelsVisible(!labels.empty());
    ...
    if (inConfig.Get(CfgID::ValueUnitsVisible, valueUnitsVisibleByName))
    {
        ...
        mpSignalWidget->Display().SetValueUnitsVisible(valueUnitsVisible);
    }
    ...
    // Sanity checks.
    if (mpSignalWidget->Display().MinValue() == mpSignalWidget->Display().MaxValue())
        mpSignalWidget->Display().SetMaxValue(mpSignalWidget->Display().MinValue() + 1);
}
...
void VisDisplayGraph::OnSignal(const GenericSignal &s)
{
    ...
    int curChannels = mNumChannels, curSamples = mpSignalWidget->Display().NumSamples();
    ...
    mpSignalWidget->Display().WrapForward(decimatedSignal);
    if (mAutoScale)
    {
        int autoScaleSamples = mpSignalWidget->Display().NumSamples();
        const PhysicalUnit& unit = decimatedSignal.Properties().ElementUnit();
        if (unit.Symbol() == "s")
            autoScaleSamples = unit.PhysicalToRaw(mAutoScaleTime);
        autoScaleSamples = std::min(mpSignalWidget->Display().NumSamples(), autoScaleSamples);
        int numChannels = mpSignalWidget->Display().NumChannels();
        ...
        for(int ch = 0; ch < numChannels; ++ch)
        {
            ...
            {
                sigMax = std::max(sigMax, mpSignalWidget->Display().Data(ch, sample));
                sigMin = std::min(sigMin, mpSignalWidget->Display().Data(ch, sample));
            }
            if (sigMin > 0)
                sigMin = 0;
            else if (sigMax < 0)
            ...
            if (sigMax != sigMin)
            {
                displayMax[ch] = sigMax;
                displayMin[ch] = sigMin;
            }
        }
        mpSignalWidget->Display().SetMaxValues(displayMax);
        mpSignalWidget->Display().SetMinValues(displayMin);
    }
    mSignalElements = s.Elements();

    if (curChannels != mNumChannels || curSamples != mpSignalWidget->Display().NumSamples())
...
void VisDisplayGraph::BuildGraphContextMenu()
{
    ...
    mpActResetSignalScale = mpGraphContextMenu->addAction("Reset Signal Scale", this, &VisDisplayGraph::ResetSignalScale);
...
void VisDisplayGraph::BuildChannelListContextMenu()
{
    ...
    mpActChannelList_Reset = pMenu->addAction("Reset Signal Scale", this, &VisDisplayGraph::ChannelList_ResetSignalScale);
    pMenu->addSeparator();
...
{
    for (int ch = 0; ch < mpSignalWidget->Display().NumChannels(); ++ch)
    {
...
{
    for (auto& scaling : mChannelUserScaling)
        scaling = 0;
    ApplyUserScaling();
}
...
void VisDisplayGraph::ApplyUserScaling()
{
    ...
    if (mMinValues.size() < channels)
        mMinValues.resize(channels, mpSignalWidget->Display().MinValue());
    if (mMaxValues.size() < channels)
        mMaxValues.resize(channels, mpSignalWidget->Display().MaxValue());
