//////////////////////////////////////////////////////////////////////
// $Id: VisRecorder.cpp 7824 2024-02-07 16:25:09Z mellinger $
...
static void BitmapToRGB(const BitmapImage& img, void* pRGBData, int width, int height)
{
    ...
    for (int x = 0; x < img.Width(); ++x)
    {
        ...
        {
            if (!img(x, y).IsBlack()) // for layers above the bottom one, black is used as transparent color
            {
                RGBColor c = img(x, y);
...
void VisRecorder::HandleBitmap(const std::string& inVisID, const BitmapImage& inImage)
{
    ...
    if (VisualizationRegistered(inVisID))
    {
