////////////////////////////////////////////////////////////////////////////////
// $Id: ContextMenuExtension.cpp 7516 2023-08-04 13:58:22Z mellinger $
    ...
    for (int i = 0; i < fileCount; ++i)
    {
        ...
        if (nameLength > 0)
        {
            ...
            if (::DragQueryFileW(hDrop, i, buf, nameLength + 1) > 0)
            {
                ...
                if (mpParent->IsBcidatfile(name))
                    mpParent->mFileNames.push_back(name);
            }
            delete[] buf;
        }
    }
    ...
    UINT uidLastCmd, UINT uFlags)
{
    ...
    ::InsertMenuA(hSubmenu, ViewerMenuEntry, MF_BYPOSITION,
    ...
    ::InsertMenuA(hSubmenu, AnalysisMenuEntry, MF_BYPOSITION,
        uidFirstCmd + AnalysisMenuEntry, "Open with BCI2000&Analysis");
    ::InsertMenuA(hSubmenu, P300ClassifierMenuEntry, MF_BYPOSITION,
...
bool ContextMenuExtension::IsBcidatfile(const std::wstring& file) const
{
    ...
    return reader.Open(com::DualString(file), 0).IsOpen();
}
