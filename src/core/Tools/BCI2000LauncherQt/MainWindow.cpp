////////////////////////////////////////////////////////////////////////////////
// $Id: MainWindow.cpp 8094 2024-05-01 15:58:11Z mellinger $
...
void MainWindow::SetupProgramsGroup()
{
    ...
    connect(ui->sourceList, &QListWidget::itemClicked, this, &MainWindow::onProgramSelectionChanged);
    ...
    connect(ui->sigprocList, &QListWidget::itemClicked, this, &MainWindow::onProgramSelectionChanged);
    ...
    connect(ui->applicationList, &QListWidget::itemClicked, this, &MainWindow::onProgramSelectionChanged);
...
void MainWindow::on_launchButton_clicked()
{
    ...
    else
    {
        ...
        command << ExtensionArgs("source").toLocal8Bit().constData();
        command << " --FileFormat=" << ui->fileFormatComboBox->currentData().toString().toLocal8Bit().constData();
        if (!ExecuteCommand(command.str().c_str(), progdir.c_str()))
            return;
    }
    ...
    else
    {
        ...
        ui->statusEdit->repaint();
        command << "\"" << progdir << item->text().toLocal8Bit().constData() << "\" --local";
        command << ExtensionArgs("sigproc").toLocal8Bit().constData();
        if (!ExecuteCommand(command.str().c_str(), progdir.c_str()))
            return;
    }
    ...
    else
    {
        ...
        ui->statusEdit->repaint();
        command << "\"" << progdir << item->text().toLocal8Bit().constData() << "\" --local";
        command << ExtensionArgs("app").toLocal8Bit().constData();
        if (!ExecuteCommand(command.str().c_str(), progdir.c_str()))
            return;
    }
