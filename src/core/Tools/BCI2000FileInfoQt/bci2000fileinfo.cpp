//////////////////////////////////////////////////////////////////////
// $Id: bci2000fileinfo.cpp 7195 2023-02-07 18:31:03Z mellinger $
...
{
    QString filename =
        QFileDialog::getOpenFileName(this, tr("Open Data File"), QDir::currentPath(), tr("BCI2000 Data Files (*.dat *.bcidat)"));
    if (!filename.isEmpty())
    {
        QDir::setCurrent(QFileInfo(filename).canonicalPath());
        DoOpenFile(filename);
...
{
    mFile.Open(filename.toLocal8Bit().constData());
    if (!mFile.IsOpen())
    {
        if (gpConfig)
            gpConfig->accept();
