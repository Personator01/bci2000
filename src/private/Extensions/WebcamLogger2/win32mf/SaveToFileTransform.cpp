////////////////////////////////////////////////////////////////////////////////
// $Id: SaveToFileTransform.cpp 7882 2024-03-01 20:34:21Z mellinger $
...
SaveToFileTransform::SaveToFileTransform()
  ...
  mpWebcamClient(nullptr), mDecimation(1), mVideoProfile(1),
...
std::string SaveToFileTransform::Error() const
{
  return mErrorStream.str();
}

void SaveToFileTransform::SetClient(WebcamClient* pClient)
{
  mpWebcamClient = pClient;
}

WebcamClient* SaveToFileTransform::Client() const
{
...
bool SaveToFileTransform::OpenFile(const std::string& filename)
{
  ...
  if (FileUtils::Exists(fullfile)) {
    mErrorStream << "Output file \"" << fullfile << "\" exists, will not be touched." << std::endl;
    return false;
  }
  if (!FileUtils::IsWritableLocation(fullfile)) {
    mErrorStream << "Output file \"" << fullfile << "\" cannot be created at the given location." << std::endl;
    return false;
  }
  ...
  UINT32 bitrate = 0.14 * mVideoInputType.width * mVideoInputType.height * mVideoInputType.framerateN / mVideoInputType.framerateD / mDecimation;
...
void SaveToFileTransform::NotifyWebcamClient(IMFSample* pSample)
{
  if (!mpWebcamClient)
    return;
  ...
  if (count > 0) {
      ...
      // Copy data from buffer into bitmap without flipping vertically.
      hr = ::MFCopyImage(mpOffscreenBufferMemory, bitmapStride,
          pData, frameStride, width * sizeof(RGBQUAD), height);
      hr = pBuffer->Unlock();

      mpWebcamClient.load()->OnWebcamFrameData(width, height, mpOffscreenBufferMemory);
  }
}
