...
#else
	MMFILE_ELECTRODE_NAME() { ::memset(this, 0, sizeof(*this)); }
	~MMFILE_ELECTRODE_NAME() { ::HeapFree(::GetProcessHeap(), 0, *pszName); }
	// from disassembly, only first element of pszName seems to be used
#endif
};
...
struct MMFILE_ELECTRODE_CODE {
	...
	NK_API void __stdcall RemoveAllItem(void);						/// Remove all the items
#else
	MMFILE_ELECTRODE_CODE() { ::memset(this, 0, sizeof(*this)); }
	~MMFILE_ELECTRODE_CODE() { ::HeapFree(::GetProcessHeap(), 0, pwCode); }
	// from disassembly, only first element of pszName seems to be used
#endif
};
