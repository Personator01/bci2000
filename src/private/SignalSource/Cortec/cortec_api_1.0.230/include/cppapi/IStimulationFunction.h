...
/**
* @brief An IStimulationFunction defines the stimulation signal that is applied between stimulation channels.
*
* A stimulation function consists of
* - a sequence of IStimulationAtoms that can be repeated a number of times.
* - and the information which channels to use for stimulation.
*
* There are two types of stimulation functions. They consist of the following atoms:
*
...
* both) and non-empty (because at least two electrodes are needed for the stimulation).
*
...
* vary, starting with index 0. In addition, depending on the actually used implant,
* the electrode indices and the allowed combinations of electrodes into virtual electrodes may differ.
* Please refer to the documentation of the used implant for more details on this.
*
* Each stimulation pulse can be executed repeatedly. The number of times a function will be repeated can be configured
* by using two parameters that both can be freely configured in the value range of 1-255:
*
* 1) Pulse repetitions
* 2) Burst repetitions
* 
* Internally, the function repetitions are organized into a burst that contains up to 255 repetitions of a pulse. The
* burst can also be repeated as a whole up to 255 times.
*
* Example execution if two pulse repetitions (PR) and two burst repetitions (BR) are configured:
*
* Execution order will be: | PR 1 | PR2 |    | PR 1 | PR 2|
*                          |     BR1    |    |     BR2    |
*
* The total number of function repetitions is the number of burst repetitions times the number of burst repetitions.
* In the example above, the pulse is therefore executed 4 times (once for each occurrence of a PR).
*
* Note that between the burst executions there is always a break of 10 us!
*
* In general, a stimulation pulse can be repeated by using only pulse repetitions. If the number of desired repetitions 
* exceeds 255, a combination of pulse repetitions and burst repetitions must be used.
*
* Typical usage:
* 1. Create the desired IStimulationFunction instance with an IStimulationCommandFactory.
* 2. Set repetitions of the function by using setRepetitions().
* 3. Set virtual stimulation electrodes.
*
*
...
class IStimulationFunction
{
    ...
    /**
    * Attributes of stimulation pulses.
    */
    enum class PulseAttributes
    {
    PA_AMPLITUDE = 0,
    PA_PULSE_WIDTH,
    PA_DZ0_DURATION,
    PA_DZ1_DURATION,
    PA_COUNT // number of types
    };

    /**
    * Append a stimulation atom to the end of the stimulation function.
    * Only valid atoms may be appended. An atom is considered invalid if one of the following statements holds:
    ...
    * Between burst repetitions there is a 10 micro seconds pause.
    *
    * @param[in] pulseRepetitions Number of times a pulse is repeated inside a burst.
    * @param[in] burstRepetitions Number of times a burst is repeated.
    *
    * \if INTERN_IMPLANTAPI
    *     @throws CInvalidArgumentException if burst or pulse repetitions < 1 or > 255.
    * \else
    *     @throws std::exception if burst or pulse repetitions < 1 or > 255.
    * \endif
          
    */
    virtual void setRepetitions(const uint32_t pulseRepetitions, const uint32_t burstRepetitions) = 0;

    /**
    * @return The number of times the sequence of atoms in a function will be repeated within a burst.
    */
    virtual uint32_t getPulseRepetitions() const = 0;

    /**
    * @return The number of times the pulse bursts will be repeated.
    */
    virtual uint32_t getBurstRepetitions() const = 0;

    /**
    * @return The total number of times the sequence of atoms in a function will be repeated.
    */
    ...
    *
    * @param functionName Name of the stimulation function.
    */
    virtual void setName(const std::string& functionName) = 0;

    /**
    * @return The set name of the function. If the function name was not set, an empty string is returned.
    */
    virtual std::string getName() const = 0;

    /**
    * @return The total duration of the function in microseconds defined by the function including the time required 
    * for all repetitions.
    */
    virtual uint64_t getDuration() const = 0;

    /**
    * The period is the duration of one function repetition, i.e. the sum of all atom durations. If (repetitions == 1),
    ...
    */
    ...
    /**
    * Set the virtual electrodes for the stimulation function. If useGndElectrode is set to true, then destinationChannels
    * may be an empty set.
    *
    * @param[in] sourceChannels      Set of source electrodes where the function is applied to,
    *                                with electrode indices in [0, CImplantInfo::getChannelCount() - 1].
    * @param[in] destinationChannels Set of electrodes that are used as destination electrodes by this function,
    *                                with electrode indices in [0, CImplantInfo::getChannelCount() - 1].
    * @param[in] useGndElectrode     True if stimulation to the ground electrode (as destination) should be used.
    *
    * \if INTERN_IMPLANTAPI
    ...
    * \else
    *   @throws std::exception if the sets are not disjunct or if destination set is empty and useGndElectrode is set to false.
    * \endif
    */
    ...
    */
    ...
    /**
    * Check if another function has the same form (i.e., consists of the same number of atoms and the atoms are 
